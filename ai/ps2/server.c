...
int main(int argc, char* argv[]) {
  ...
  client_sock[0] =
    accept(socket_desc, (struct sockaddr*)&client_addr1, &client_size);
  if(client_sock[0] < 0) {
    printf("Can't accept\n");
    return -1;
  }
  ...
  client_sock[1] =
    accept(socket_desc, (struct sockaddr*)&client_addr2, &client_size);
  if(client_sock[1] < 0) {
    printf("Can't accept\n");
    return -1;
  }
  ...
  while((moveNo < 25) && (!isWin) && (!isLose) && correct) {
    ...
    printf("Move no: %d\n", moveNo);
    memset(client_message, '\0', sizeof(client_message));
    if(recv(client_sock[sockNo], client_message, sizeof(client_message), 0) <
       0) {
      printf("Couldn't receive\n");
      return -1;
    }
    ...
    if(correct)
      isWin = winCheck(player);
    if(correct && !isWin)
      isLose = loseCheck(player);
    ...
    player = 3 - player;
    sockNo = 1 - sockNo;
    if(send(client_sock[sockNo], server_message, strlen(server_message), 0) <
       0) {
      printf("Can't send\n");
      return -1;
    }
