...
[[nodiscard]] static u8 read_uint8(u8 const* const stream, i64& pos)
{
  pos += 1;
  return *(stream + pos - 1);
}
...
[[nodiscard]] static u16 read_uint16_le(u8 const* const stream)
{
  return static_cast<u16>(stream[0]) | (static_cast<u16>(stream[1]) << 8);
}

[[nodiscard]] static u32 read_uint32_le(u8 const* const stream)
{
  return static_cast<u32>(stream[0]) | (static_cast<u32>(stream[1]) << 8) |
         (static_cast<u32>(stream[2]) << 16) |
         (static_cast<u32>(stream[3]) << 24);
}

[[nodiscard]] static u16 read_uint16_le(u8 const* const stream, i64& pos)
{
  pos += 2;
  return read_uint16_le(stream + pos - 2);
}

[[nodiscard]] static u32 read_uint32_le(u8 const* const stream, i64& pos)
{
  pos += 4;
...
[[nodiscard]] static u16 read_uint16_be(u8 const* const stream)
{
  return (static_cast<u16>(stream[0]) << 8) | static_cast<u16>(stream[1]);
}

[[nodiscard]] static u32 read_uint32_be(u8 const* const stream)
{
  return (static_cast<u32>(stream[0]) << 24) |
         (static_cast<u32>(stream[1]) << 16) |
         (static_cast<u32>(stream[2]) << 8) | static_cast<u32>(stream[3]);
}

[[nodiscard]] static u64 read_uint64_be(u8 const* const stream)
{
  return (static_cast<u64>(stream[0]) << 56) |
         (static_cast<u64>(stream[1]) << 48) |
         (static_cast<u64>(stream[2]) << 40) |
         (static_cast<u64>(stream[3]) << 32) |
         (static_cast<u64>(stream[4]) << 24) |
         (static_cast<u64>(stream[5]) << 16) |
         (static_cast<u64>(stream[6]) << 8) | static_cast<u64>(stream[7]);
}

[[nodiscard]] static u64 read_uint64_be(u8 const* const stream, i64& pos)
{
  pos += 8;
...
// constexpr u32 extension_color_correction_offset_offset = 482; // Currently
// unused constexpr u32 extension_postage_stamp_offset_offset = 486; //
// Currently unused constexpr u32 extension_scan_line_offset_offset = 490; //
// Currently unused constexpr u32 extension_attributes_type_offset = 494; //
// Currently unused
...
[[nodiscard]] static TGA_Footer read_footer(u8 const* const stream, i64& pos)
{
  TGA_Footer footer;
...
// TGA2 test
bool test_tga(Array<u8> const& stream)
{
  i64 pos =
    stream.size() -
...
Expected<Image, Error> import_tga(Array<u8> const& tga_data)
{
  ...
  if(header.pixel_depth != 8 && header.pixel_depth != 16 &&
    ...
    return {expected_error, Error("Unsupported pixel depth")};
  }

  if(header.image_type == img_type_no_image_data) {
    return {expected_error, Error("File contains no image data")};
  }
  ...
  case 8:
    if(is_greyscale && alpha_bits == 0) {
      ...
      return {expected_error, Error("Unknown pixel format")};
    }
    break;
  case 16:
    // TODO: add rgb5_a1
  ...
  auto swap_and_copy_bytes =
    [alpha_bits](u8* const out, u8 const* const in, i64 const bytes_to_copy,
      ...
      for(i64 offset = 0; offset < bytes_to_copy; offset += bytes_per_chunk) {
        ...
        case 2:
          if(alpha_bits == 8) {
            // Just copy because the color is stored as RA in little-endian and we
            // want RA format
            out[offset] = in[offset];
        ...
        case 4:
          if(!indexed) {
            // We want to get the color as RGBA, so we only swap RGB bytes and
            // leave alpha byte unmodified
            out[offset] = in[offset + 2];
            out[offset + 1] = in[offset + 1];
            out[offset + 2] = in[offset];
  ...
  // We cannot simply copy the data from the file to our storage because tga is
  // the dumbest format ever where non-related bytes are stored in
  // little-endian, a.k.a. RGB is BGR. We have to perform a lot of byte
  // swapping, bit shifting and what not.
    ...
    for(i64 offset = 0, img_offset = 0; img_offset < image_data_length;) {
      ...
      if(is_RL_packet) {
        ...
        for(i32 j = 1; j < repeat_count; ++j) {
          copy(swapped_pixel, swapped_pixel + bytes_per_pixel,
  ...
  if(is_indexed) {
    // Deindexing
    Array<u8> colors(header.image_width * header.image_height * pixel_width);
    ...
    for(i64 offset = 0; offset < image.data.size();) {
      ...
      case 3:
        index =
          (static_cast<u64>(read_uint16_be(indices_ptr)) << 8) + indices_ptr[2];
        break;
      case 4:
      ...
      swap_and_copy_bytes(colors_ptr + offset, color_map + index * pixel_width,
                          bytes_per_color_map_pixel, bytes_per_color_map_pixel,
                          false);
  ...
  // TGA's are in the old format, hence no footer.
  ...
  //             Error("TGA extension size is not 495. Unknown TGA format")};
  //   }

  //   gamma_numerator = read_uint16_le(tga_stream +
  //   footer.extension_area_offset +
  ...
  return {expected_value, ANTON_MOV(image)};
}
