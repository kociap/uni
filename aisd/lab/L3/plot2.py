...
data_info = [
    { "name": "select", "path": lambda k, i: f"./o/select_data/out_{k}_{100 * i}.txt"},
    { "name": "randselect", "path": lambda k, i: f"./o/select_data/randout_{k}_{100 * i}.txt"},
]
...
for info in data_info:
    for k in (1, 10, 50, 99):
        ...
        times.insert(len(times.columns), f"{info['name']} (k={k})", means.iloc[:, 0])
        comparisons.insert(len(comparisons.columns), f"{info['name']} (k={k})", means.iloc[:, 1])
        swaps.insert(len(swaps.columns), f"{info['name']} (k={k})", means.iloc[:, 2])

times_axes = times.plot()
times_axes.set_ylabel("time (ns)")
times_axes.figure.savefig("./select_time.png")
print(times)
comparisons_axes = comparisons.plot()
comparisons_axes.set_ylabel("comparisons")
comparisons_axes.figure.savefig("./select_compares.png")
print(comparisons)
swaps_axes = swaps.plot()
swaps_axes.set_ylabel("swaps")
swaps_axes.figure.savefig("./select_swaps.png")
print(comparisons)
