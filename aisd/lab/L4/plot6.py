...
for index in range(1, 11):
    data = read_data_file(f"./o/out_splay_rand_{10000 * index}.txt")
    figure, axes = create_plot(data)
    axes.set_xticks(ticks = [], labels = [])
    figure.savefig(f"./splay_rand_{10000 * index}.png")

for index in range(1, 11):
    data = read_data_file(f"./o/out_splay_lin_{10000 * index}.txt")
    figure, axes = create_plot(data)
    axes.set_xticks(ticks = [], labels = [])
    axes.set_ylim(ymin = 0, ymax = 10000 * index)
    figure.savefig(f"./splay_lin_{10000 * index}.png")