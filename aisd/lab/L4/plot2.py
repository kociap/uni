...
for index in range(1, 11):
    data = read_data_file(f"./o/out_rand_{10000 * index}.txt")
    figure, axes = create_plot(data)
    axes.set_xticks(ticks = [], labels = [])
    figure.savefig(f"./bst_rand_{10000 * index}.png")

for index in range(1, 11):
    data = read_data_file(f"./o/out_lin_{10000 * index}.txt")
    figure, axes = create_plot(data)
    axes.set_xticks(ticks = [], labels = [])
    figure.savefig(f"./bst_lin_{10000 * index}.png")